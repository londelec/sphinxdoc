...
from roles import build_bitrefs
from wrapper import *

COMMENT_DRAWING_CHARS = '-=_~.*`\'"+'

class fieldlisttable(nodes.General, nodes.Element):
    pass
...
def setup(app):
    app.add_node(fieldlisttable,
                 html=(skip_node, skip_node),
                 singlehtml=(skip_node, skip_node),
                 latex=(skip_node, skip_node),
                 text=(skip_node, skip_node))

    add_node_wrapper(app, nodes.entry,
            html=(html_visit_entry, html_depart_entry),
            singlehtml=(html_visit_entry, html_depart_entry))

    app.add_directive('field-list-table', FieldListTable)
...
def yes_no_zero_one(argument):
    return directives.choice(argument, ('yes', 'no', '0', '1', 'true', 'false'))
...
#        'label'          : directives.uri,
    ...
    def run2(self):
        ...
        if self.options.get('transformation') in ['no', '0', 'false']:
        ...
        if not isinstance(self.node[0],nodes.bullet_list):
            msg = ("Content type is wrong. Exactly one bullet list "
                   "is expected.")
            raise FieldListTableError(msg)
        if self.options.get('definition-row') in ['yes', '1', 'true']:
        ...
        fieldListTable_node = fieldlisttable('', ids=[label] if label is not None else [])

        spec = self.options.get('spec', None)
        if spec is not None:
            table_spec_node = addnodes.tabular_col_spec()
            table_spec_node['spec'] = spec
            fieldListTable_node.append(table_spec_node)

        fieldListTable_node.append(tableNode)
        build_bitrefs(self, tableNode)

        return [fieldListTable_node] + messages
        ...
        for self.rowNum, row in enumerate(self.tableData):
            ...
            for self.colNum, cell in enumerate(row):
                ...
                if info.get('isInColspan'):
                    ...
                    entry = nodes.entry()
                    if self.options.get('debug-cellinfo') in ['yes', '1', 'true']:
                        interesting = [
                            (1,'colNum'),
                            (1,'rowNum'),
                            (0,'colwidth'),
            ...
            rows.append(rowNode)
        if headerRows:
            headerRowList = rows[:headerRows]

            if not self.options.get('bold-header') in ['no', '0', 'false']:
                for row in enumerate(headerRowList):
                    for textNode in row[1].traverse(nodes.Text):
                        strongNode = nodes.strong('', textNode)
                        parent = textNode.parent
                        textNode.parent.replace(textNode, strongNode)

            thead = nodes.thead()
            thead.extend(headerRowList)
            tgroup += thead
        tbody = nodes.tbody()
...
"""
This function overrides visit_entry() defined in
/usr/lib/pythonX/dist-packages/docutils/writers/_html_base.py
It is called for every table row <entry> encountered in the document tree
Alignment values are parsed by isValidAlignment() and added to row entry attributes e.g. <entry align="left">
They are not used by docutils, but we convert them to classes in this function.
Result will be <td class"align-left">xxx</td>
"""
def html_visit_entry(self, node):
    atts = {'class': []}
    tagname = 'td'
    ...
    if align:
        for aval in align.split(' '):
            atts['class'].append('align-' + aval)
...
def html_depart_entry(self, node):
